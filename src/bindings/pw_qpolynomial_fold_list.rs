...
use libc::uintptr_t;
use crate::bindings::{Stat, PwQPolynomialFold, Context};
...
impl Clone for PwQPolynomialFoldList {
    fn clone(&self) -> PwQPolynomialFoldList {
        self.copy()
    }
}

impl FromIterator<PwQPolynomialFold> for PwQPolynomialFoldList {
    fn from_iter<T>(iter: T) -> Self
    where T: IntoIterator<Item = PwQPolynomialFold>,
    {
...
impl PwQPolynomialFoldList {
    ...
    pub fn map<fn_>(self, mut fn_: fn_, user: *mut c_void) -> PwQPolynomialFoldList
    ...
    pub fn sort<cmp>(self, mut cmp: cmp, user: *mut c_void) -> PwQPolynomialFoldList
    ...
    pub fn set_pw_qpolynomial_fold(self, index: i32, el: PwQPolynomialFold) -> PwQPolynomialFoldList {
        let context_for_error_message = self.get_ctx();
        let list = self;
        let mut list = list;
        list.do_not_free_on_drop();
...
impl Drop for PwQPolynomialFoldList {
    fn drop(&mut self) {
        if self.should_free_on_drop {
            unsafe { isl_pw_qpolynomial_fold_list_free(self.ptr); }
        }
    }
}