...
use libc::uintptr_t;
use crate::bindings::{Stat, Aff, Context};
...
extern "C" {
    ...
    fn isl_aff_list_foreach_scc(list: uintptr_t, follows: unsafe extern "C" fn(uintptr_t, uintptr_t, *mut c_void) -> i32, follows_user: *mut c_void, fn_: unsafe extern "C" fn(uintptr_t, *mut c_void) -> Stat, fn_user: *mut c_void) -> Stat;
    ...
    fn isl_aff_list_foreach(list: uintptr_t, fn_: unsafe extern "C" fn(uintptr_t, *mut c_void) -> Stat, user: *mut c_void) -> Stat;

    fn isl_aff_list_every(list: uintptr_t, test: unsafe extern "C" fn(uintptr_t, *mut c_void) -> i32, user: *mut c_void) -> i32;

    fn isl_aff_list_map(list: uintptr_t, fn_: unsafe extern "C" fn(uintptr_t, *mut c_void) -> uintptr_t, user: *mut c_void) -> uintptr_t;
    ...
    fn isl_aff_list_sort(list: uintptr_t, cmp: unsafe extern "C" fn(uintptr_t, uintptr_t, *mut c_void) -> i32, user: *mut c_void) -> uintptr_t;
...
impl AffList {
          ...
          fn_: FnMut(&AffList) -> Stat,
    {
        ...
        let isl_rs_result = unsafe { isl_aff_list_foreach_scc(list, follows, follows_user, fn_, fn_user) };
        isl_rs_result
    }
    ...
    pub fn map<fn_>(self, mut fn_: fn_, user: *mut c_void) -> AffList
    ...
    pub fn sort<cmp>(self, mut cmp: cmp, user: *mut c_void) -> AffList
...
impl Drop for AffList {
    fn drop(&mut self) {
        if self.should_free_on_drop {
            unsafe { isl_aff_list_free(self.ptr); }
        }
    }
}