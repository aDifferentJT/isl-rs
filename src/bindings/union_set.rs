...
use crate::bindings::{DimType, Stat, Space, BasicSet, Set, Context, MultiAff, Point, BasicSetList, UnionMap, PwMultiAff, UnionSetList, SetList, UnionPwMultiAff};
...
extern "C" {
    ...
    fn isl_union_set_foreach_point(uset: uintptr_t, fn_: unsafe extern "C" fn(uintptr_t, *mut c_void) -> Stat, user: *mut c_void) -> Stat;

    fn isl_union_set_foreach_set(uset: uintptr_t, fn_: unsafe extern "C" fn(uintptr_t, *mut c_void) -> Stat, user: *mut c_void) -> Stat;
    ...
    fn isl_union_set_every_set(uset: uintptr_t, test: unsafe extern "C" fn(uintptr_t, *mut c_void) -> i32, user: *mut c_void) -> i32;
...
impl UnionSet {
    ...
    pub fn foreach_set<fn_>(&self, mut fn_: fn_, user: *mut c_void) -> Stat
    ...
    pub fn every_set<test>(&self, mut test: test, user: *mut c_void) -> bool
...
impl Drop for UnionSet {
    fn drop(&mut self) {
        if self.should_free_on_drop {
            unsafe { isl_union_set_free(self.ptr); }
        }
    }
}