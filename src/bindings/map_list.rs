...
use crate::bindings::{Stat, Map, Context};
...
extern "C" {
    ...
    fn isl_map_list_map(list: uintptr_t, fn_: unsafe extern "C" fn(uintptr_t, *mut c_void) -> uintptr_t, user: *mut c_void) -> uintptr_t;
    ...
    fn isl_map_list_foreach(list: uintptr_t, fn_: unsafe extern "C" fn(uintptr_t, *mut c_void) -> Stat, user: *mut c_void) -> Stat;
    ...
    fn isl_map_list_every(list: uintptr_t, test: unsafe extern "C" fn(uintptr_t, *mut c_void) -> i32, user: *mut c_void) -> i32;
    ...
    fn isl_map_list_sort(list: uintptr_t, cmp: unsafe extern "C" fn(uintptr_t, uintptr_t, *mut c_void) -> i32, user: *mut c_void) -> uintptr_t;

    fn isl_map_list_foreach_scc(list: uintptr_t, follows: unsafe extern "C" fn(uintptr_t, uintptr_t, *mut c_void) -> i32, follows_user: *mut c_void, fn_: unsafe extern "C" fn(uintptr_t, *mut c_void) -> Stat, fn_user: *mut c_void) -> Stat;
...
impl MapList {
    ...
    pub fn map<fn_>(self, mut fn_: fn_, user: *mut c_void) -> MapList
    where fn_: FnMut(&Map) -> &Map,
    {
        let context_for_error_message = self.get_ctx();
        let list = self;
        let mut list = list;
    ...
    pub fn sort<cmp>(self, mut cmp: cmp, user: *mut c_void) -> MapList
    where cmp: FnMut(&Map, &Map) -> i32,
    {
        let context_for_error_message = self.get_ctx();
        let list = self;
        let mut list = list;
    ...
    where follows: FnMut(&Map, &Map) -> bool,
          fn_: FnMut(&MapList) -> Stat,
    {
        let context_for_error_message = self.get_ctx();
        let list = self;
        let list = list.ptr;
        ...
        let user: *mut c_void = unsafe { core::mem::transmute(user) };
        let fn_ = fn__wrapper::<fn_>;
        let isl_rs_result = unsafe { isl_map_list_foreach_scc(list, follows, follows_user, fn_, fn_user) };
        isl_rs_result
    }
...
impl Drop for MapList {
    fn drop(&mut self) {
        if self.should_free_on_drop {
            unsafe { isl_map_list_free(self.ptr); }
        }
    }
}