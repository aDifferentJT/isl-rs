...
use libc::uintptr_t;
use crate::bindings::{Stat, UnionPwMultiAff, Context};
...
extern "C" {
    ...
    fn isl_union_pw_multi_aff_list_get_union_pw_multi_aff(list: uintptr_t, index: i32) -> uintptr_t;

    fn isl_union_pw_multi_aff_list_foreach(list: uintptr_t, fn_: unsafe extern "C" fn(uintptr_t, *mut c_void) -> Stat, user: *mut c_void) -> Stat;

    fn isl_union_pw_multi_aff_list_every(list: uintptr_t, test: unsafe extern "C" fn(uintptr_t, *mut c_void) -> i32, user: *mut c_void) -> i32;

    fn isl_union_pw_multi_aff_list_map(list: uintptr_t, fn_: unsafe extern "C" fn(uintptr_t, *mut c_void) -> uintptr_t, user: *mut c_void) -> uintptr_t;

    fn isl_union_pw_multi_aff_list_foreach_scc(list: uintptr_t, follows: unsafe extern "C" fn(uintptr_t, uintptr_t, *mut c_void) -> i32, follows_user: *mut c_void, fn_: unsafe extern "C" fn(uintptr_t, *mut c_void) -> Stat, fn_user: *mut c_void) -> Stat;
    ...
    fn isl_union_pw_multi_aff_list_set_union_pw_multi_aff(list: uintptr_t, index: i32, el: uintptr_t) -> uintptr_t;
    ...
    fn isl_union_pw_multi_aff_list_sort(list: uintptr_t, cmp: unsafe extern "C" fn(uintptr_t, uintptr_t, *mut c_void) -> i32, user: *mut c_void) -> uintptr_t;
...
impl Clone for UnionPwMultiAffList {
    fn clone(&self) -> UnionPwMultiAffList {
        self.copy()
    }
}

impl FromIterator<UnionPwMultiAff> for UnionPwMultiAffList {
    fn from_iter<T>(iter: T) -> Self
    where T: IntoIterator<Item = UnionPwMultiAff>,
    {
...
impl UnionPwMultiAffList {
    ...
    pub fn get_union_pw_multi_aff(&self, index: i32) -> UnionPwMultiAff {
        ...
        let list = self;
        let list = list.ptr;
        let isl_rs_result = unsafe { isl_union_pw_multi_aff_list_get_union_pw_multi_aff(list, index) };
        if isl_rs_result == 0 {
            panic!("ISL error: {}", context_for_error_message.last_error_msg());
        }
        let isl_rs_result = UnionPwMultiAff { ptr: isl_rs_result, should_free_on_drop: true };
    ...
    /// Wraps `isl_union_pw_multi_aff_list_foreach`.
    pub fn foreach<fn_>(&self, mut fn_: fn_, user: *mut c_void) -> Stat
    where fn_: FnMut(&UnionPwMultiAff) -> Stat,
    {
        let context_for_error_message = self.get_ctx();
        let list = self;
        let list = list.ptr;
        ...
        let isl_rs_result = unsafe { isl_union_pw_multi_aff_list_foreach(list, fn_, user) };
        isl_rs_result
    }

    /// Wraps `isl_union_pw_multi_aff_list_every`.
    pub fn every<test>(&self, mut test: test, user: *mut c_void) -> bool
    where test: FnMut(&UnionPwMultiAff) -> bool,
    {
        let context_for_error_message = self.get_ctx();
        let list = self;
        let list = list.ptr;
        ...
        let isl_rs_result = unsafe { isl_union_pw_multi_aff_list_every(list, test, user) };
    ...
    /// Wraps `isl_union_pw_multi_aff_list_map`.
    pub fn map<fn_>(self, mut fn_: fn_, user: *mut c_void) -> UnionPwMultiAffList
    where fn_: FnMut(&UnionPwMultiAff) -> &UnionPwMultiAff,
    {
        let context_for_error_message = self.get_ctx();
        let list = self;
        let mut list = list;
        ...
        let user: *mut c_void = unsafe { core::mem::transmute(user) };
        let fn_ = fn__wrapper::<fn_>;
        let isl_rs_result = unsafe { isl_union_pw_multi_aff_list_map(list, fn_, user) };
        if isl_rs_result == 0 {
            panic!("ISL error: {}", context_for_error_message.last_error_msg());
        }
    ...
    /// Wraps `isl_union_pw_multi_aff_list_foreach_scc`.
    pub fn foreach_scc<follows, fn_>(&self, mut follows: follows, follows_user: *mut c_void, mut fn_: fn_, fn_user: *mut c_void) -> Stat
    where follows: FnMut(&UnionPwMultiAff, &UnionPwMultiAff) -> bool,
          fn_: FnMut(&UnionPwMultiAffList) -> Stat,
    {
        let context_for_error_message = self.get_ctx();
        let list = self;
        let list = list.ptr;
        ...
        let user: *mut c_void = unsafe { core::mem::transmute(user) };
        let fn_ = fn__wrapper::<fn_>;
        let isl_rs_result = unsafe { isl_union_pw_multi_aff_list_foreach_scc(list, follows, follows_user, fn_, fn_user) };
        isl_rs_result
    }
    ...
    pub fn set_union_pw_multi_aff(self, index: i32, el: UnionPwMultiAff) -> UnionPwMultiAffList {
        ...
        let el = el.ptr;
        let isl_rs_result = unsafe { isl_union_pw_multi_aff_list_set_union_pw_multi_aff(list, index, el) };
        if isl_rs_result == 0 {
            panic!("ISL error: {}", context_for_error_message.last_error_msg());
        }
        let isl_rs_result = UnionPwMultiAffList { ptr: isl_rs_result, should_free_on_drop: true };
    ...
    /// Wraps `isl_union_pw_multi_aff_list_sort`.
    pub fn sort<cmp>(self, mut cmp: cmp, user: *mut c_void) -> UnionPwMultiAffList
    where cmp: FnMut(&UnionPwMultiAff, &UnionPwMultiAff) -> i32,
    {
        let context_for_error_message = self.get_ctx();
        let list = self;
        let mut list = list;
        ...
        let isl_rs_result = unsafe { isl_union_pw_multi_aff_list_sort(list, cmp, user) };
        if isl_rs_result == 0 {
            panic!("ISL error: {}", context_for_error_message.last_error_msg());
        }
        let isl_rs_result = UnionPwMultiAffList { ptr: isl_rs_result, should_free_on_drop: true };
...
impl Drop for UnionPwMultiAffList {
    fn drop(&mut self) {
        if self.should_free_on_drop {
            unsafe { isl_union_pw_multi_aff_list_free(self.ptr); }
        }
    }
}