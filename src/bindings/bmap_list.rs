...
use crate::bindings::{Stat, BasicMap, Context};
...
extern "C" {
    ...
    fn isl_basic_map_list_every(list: uintptr_t, test: unsafe extern "C" fn(uintptr_t, *mut c_void) -> i32, user: *mut c_void) -> i32;
    ...
    fn isl_basic_map_list_foreach_scc(list: uintptr_t, follows: unsafe extern "C" fn(uintptr_t, uintptr_t, *mut c_void) -> i32, follows_user: *mut c_void, fn_: unsafe extern "C" fn(uintptr_t, *mut c_void) -> Stat, fn_user: *mut c_void) -> Stat;

    fn isl_basic_map_list_sort(list: uintptr_t, cmp: unsafe extern "C" fn(uintptr_t, uintptr_t, *mut c_void) -> i32, user: *mut c_void) -> uintptr_t;
    ...
    fn isl_basic_map_list_foreach(list: uintptr_t, fn_: unsafe extern "C" fn(uintptr_t, *mut c_void) -> Stat, user: *mut c_void) -> Stat;

    fn isl_basic_map_list_map(list: uintptr_t, fn_: unsafe extern "C" fn(uintptr_t, *mut c_void) -> uintptr_t, user: *mut c_void) -> uintptr_t;
...
impl Clone for BasicMapList {
    fn clone(&self) -> BasicMapList {
        self.copy()
    }
}

impl FromIterator<BasicMap> for BasicMapList {
    fn from_iter<T>(iter: T) -> Self
    where T: IntoIterator<Item = BasicMap>,
    {
...
impl BasicMapList {
    /// Wraps `isl_basic_map_list_every`.
    pub fn every<test>(&self, mut test: test, user: *mut c_void) -> bool
    where test: FnMut(&BasicMap) -> bool,
    {
        ...
        let isl_rs_result = unsafe { isl_basic_map_list_every(list, test, user) };
          ...
          fn_: FnMut(&BasicMapList) -> Stat,
    {
        ...
        let isl_rs_result = unsafe { isl_basic_map_list_foreach_scc(list, follows, follows_user, fn_, fn_user) };
        isl_rs_result
    }

    /// Wraps `isl_basic_map_list_sort`.
    pub fn sort<cmp>(self, mut cmp: cmp, user: *mut c_void) -> BasicMapList
    where cmp: FnMut(&BasicMap, &BasicMap) -> i32,
    {
        let context_for_error_message = self.get_ctx();
        let list = self;
        let mut list = list;
        ...
        let isl_rs_result = unsafe { isl_basic_map_list_sort(list, cmp, user) };
        if isl_rs_result == 0 {
            panic!("ISL error: {}", context_for_error_message.last_error_msg());
        }
        let isl_rs_result = BasicMapList { ptr: isl_rs_result, should_free_on_drop: true };
    ...
    /// Wraps `isl_basic_map_list_foreach`.
    ...
    where fn_: FnMut(&BasicMap) -> Stat,
    {
        ...
        let user: *mut c_void = unsafe { core::mem::transmute(user) };
        let fn_ = fn__wrapper::<fn_>;
        let isl_rs_result = unsafe { isl_basic_map_list_foreach(list, fn_, user) };
        isl_rs_result
    }

    /// Wraps `isl_basic_map_list_map`.
    pub fn map<fn_>(self, mut fn_: fn_, user: *mut c_void) -> BasicMapList
    where fn_: FnMut(&BasicMap) -> &BasicMap,
    {
        let context_for_error_message = self.get_ctx();
        let list = self;
        let mut list = list;
        ...
        let user: *mut c_void = unsafe { core::mem::transmute(user) };
        let fn_ = fn__wrapper::<fn_>;
        let isl_rs_result = unsafe { isl_basic_map_list_map(list, fn_, user) };
        if isl_rs_result == 0 {
            panic!("ISL error: {}", context_for_error_message.last_error_msg());
        }
        let isl_rs_result = BasicMapList { ptr: isl_rs_result, should_free_on_drop: true };
...
impl Drop for BasicMapList {
    fn drop(&mut self) {
        if self.should_free_on_drop {
            unsafe { isl_basic_map_list_free(self.ptr); }
        }
    }
}