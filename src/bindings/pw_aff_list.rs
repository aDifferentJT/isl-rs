...
use crate::bindings::{Stat, Context, Set, PwAff};
...
extern "C" {
    ...
    fn isl_pw_aff_list_sort(list: uintptr_t, cmp: unsafe extern "C" fn(uintptr_t, uintptr_t, *mut c_void) -> i32, user: *mut c_void) -> uintptr_t;
    ...
    fn isl_pw_aff_list_map(list: uintptr_t, fn_: unsafe extern "C" fn(uintptr_t, *mut c_void) -> uintptr_t, user: *mut c_void) -> uintptr_t;

    fn isl_pw_aff_list_max(list: uintptr_t) -> uintptr_t;

    fn isl_pw_aff_list_foreach_scc(list: uintptr_t, follows: unsafe extern "C" fn(uintptr_t, uintptr_t, *mut c_void) -> i32, follows_user: *mut c_void, fn_: unsafe extern "C" fn(uintptr_t, *mut c_void) -> Stat, fn_user: *mut c_void) -> Stat;

    fn isl_pw_aff_list_every(list: uintptr_t, test: unsafe extern "C" fn(uintptr_t, *mut c_void) -> i32, user: *mut c_void) -> i32;
    ...
    fn isl_pw_aff_list_foreach(list: uintptr_t, fn_: unsafe extern "C" fn(uintptr_t, *mut c_void) -> Stat, user: *mut c_void) -> Stat;
...
impl Clone for PwAffList {
    fn clone(&self) -> PwAffList {
        self.copy()
    }
}

impl FromIterator<PwAff> for PwAffList {
    fn from_iter<T>(iter: T) -> Self
    where T: IntoIterator<Item = PwAff>,
    {
...
impl PwAffList {
    ...
    /// Wraps `isl_pw_aff_list_sort`.
    pub fn sort<cmp>(self, mut cmp: cmp, user: *mut c_void) -> PwAffList
    where cmp: FnMut(&PwAff, &PwAff) -> i32,
    {
        let context_for_error_message = self.get_ctx();
        let list = self;
        let mut list = list;
        ...
        let isl_rs_result = unsafe { isl_pw_aff_list_sort(list, cmp, user) };
        if isl_rs_result == 0 {
            panic!("ISL error: {}", context_for_error_message.last_error_msg());
        }
        let isl_rs_result = PwAffList { ptr: isl_rs_result, should_free_on_drop: true };
    ...
    /// Wraps `isl_pw_aff_list_map`.
    pub fn map<fn_>(self, mut fn_: fn_, user: *mut c_void) -> PwAffList
    where fn_: FnMut(&PwAff) -> &PwAff,
    {
        let context_for_error_message = self.get_ctx();
        let list = self;
        let mut list = list;
        ...
        let isl_rs_result = unsafe { isl_pw_aff_list_map(list, fn_, user) };
        if isl_rs_result == 0 {
            panic!("ISL error: {}", context_for_error_message.last_error_msg());
        }
        let isl_rs_result = PwAffList { ptr: isl_rs_result, should_free_on_drop: true };
          ...
          fn_: FnMut(&PwAffList) -> Stat,
    {
        ...
        let user: *mut c_void = unsafe { core::mem::transmute(user) };
        let fn_ = fn__wrapper::<fn_>;
        let isl_rs_result = unsafe { isl_pw_aff_list_foreach_scc(list, follows, follows_user, fn_, fn_user) };
        isl_rs_result
    }

    /// Wraps `isl_pw_aff_list_every`.
    ...
    where test: FnMut(&PwAff) -> bool,
    {
        ...
        let isl_rs_result = unsafe { isl_pw_aff_list_every(list, test, user) };
    ...
    where fn_: FnMut(&PwAff) -> Stat,
    {
        ...
        let user: *mut c_void = unsafe { core::mem::transmute(user) };
        let fn_ = fn__wrapper::<fn_>;
        let isl_rs_result = unsafe { isl_pw_aff_list_foreach(list, fn_, user) };
        isl_rs_result
    }
...
impl Drop for PwAffList {
    fn drop(&mut self) {
        if self.should_free_on_drop {
            unsafe { isl_pw_aff_list_free(self.ptr); }
        }
    }
}