...
use crate::bindings::{DimType, LocalSpace, Val, Aff, ConstraintList, Space, Context};
...
extern "C" {
    ...
    fn isl_constraint_get_dim_name(constraint: uintptr_t, type_: DimType, pos: u32) -> *const c_char;
    ...
    fn isl_constraint_involves_dims(constraint: uintptr_t, type_: DimType, first: u32, n: u32) -> i32;

    fn isl_constraint_set_coefficient_si(constraint: uintptr_t, type_: DimType, pos: i32, v: i32) -> uintptr_t;
    ...
    fn isl_constraint_get_coefficient_val(constraint: uintptr_t, type_: DimType, pos: i32) -> uintptr_t;
    ...
    fn isl_constraint_set_coefficient_val(constraint: uintptr_t, type_: DimType, pos: i32, v: uintptr_t) -> uintptr_t;
...
impl Constraint {
    ...
    pub fn set_coefficient_val(self, type_: DimType, pos: i32, v: Val) -> Constraint {
        ...
        v.do_not_free_on_drop();
        let v = v.ptr;
        let isl_rs_result = unsafe { isl_constraint_set_coefficient_val(constraint, type_, pos, v) };
        if isl_rs_result == 0 {
            panic!("ISL error: {}", context_for_error_message.last_error_msg());
        }
        let isl_rs_result = Constraint { ptr: isl_rs_result, should_free_on_drop: true };
...
impl Drop for Constraint {
    fn drop(&mut self) {
        if self.should_free_on_drop {
            unsafe { isl_constraint_free(self.ptr); }
        }
    }
}