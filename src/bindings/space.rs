...
use crate::bindings::{DimType, Context, Id};
...
extern "C" {
    ...
    fn isl_space_match(space1: uintptr_t, type1: DimType, space2: uintptr_t, type2: DimType) -> i32;
    ...
    fn isl_space_add_named_tuple_id_ui(space: uintptr_t, tuple_id: uintptr_t, dim: u32) -> uintptr_t;
    ...
    fn isl_space_tuple_is_equal(space1: uintptr_t, type1: DimType, space2: uintptr_t, type2: DimType) -> i32;

    fn isl_space_can_range_curry(space: uintptr_t) -> i32;

    fn isl_space_range_reverse(space: uintptr_t) -> uintptr_t;

    fn isl_space_move_dims(space: uintptr_t, dst_type: DimType, dst_pos: u32, src_type: DimType, src_pos: u32, n: u32) -> uintptr_t;

    fn isl_space_domain_product(left: uintptr_t, right: uintptr_t) -> uintptr_t;

    fn isl_space_range(space: uintptr_t) -> uintptr_t;

    fn isl_space_set_dim_name(space: uintptr_t, type_: DimType, pos: u32, name: *const c_char) -> uintptr_t;
    ...
    fn isl_space_set_dim_id(space: uintptr_t, type_: DimType, pos: u32, id: uintptr_t) -> uintptr_t;
...
impl Space {
    ...
    pub fn move_dims(self, dst_type: DimType, dst_pos: u32, src_type: DimType, src_pos: u32, n: u32) -> Space {
        ...
        space.do_not_free_on_drop();
        let space = space.ptr;
        let isl_rs_result = unsafe { isl_space_move_dims(space, dst_type, dst_pos, src_type, src_pos, n) };
        if isl_rs_result == 0 {
            panic!("ISL error: {}", context_for_error_message.last_error_msg());
        }
        let isl_rs_result = Space { ptr: isl_rs_result, should_free_on_drop: true };
...
impl Drop for Space {
    fn drop(&mut self) {
        if self.should_free_on_drop {
            unsafe { isl_space_free(self.ptr); }
        }
    }
}