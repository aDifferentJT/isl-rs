...
use libc::uintptr_t;
use crate::bindings::{Stat, Constraint, Context};
...
extern "C" {
    ...
    fn isl_constraint_list_foreach(list: uintptr_t, fn_: unsafe extern "C" fn(uintptr_t, *mut c_void) -> Stat, user: *mut c_void) -> Stat;
    ...
    fn isl_constraint_list_sort(list: uintptr_t, cmp: unsafe extern "C" fn(uintptr_t, uintptr_t, *mut c_void) -> i32, user: *mut c_void) -> uintptr_t;
    ...
    fn isl_constraint_list_every(list: uintptr_t, test: unsafe extern "C" fn(uintptr_t, *mut c_void) -> i32, user: *mut c_void) -> i32;

    fn isl_constraint_list_foreach_scc(list: uintptr_t, follows: unsafe extern "C" fn(uintptr_t, uintptr_t, *mut c_void) -> i32, follows_user: *mut c_void, fn_: unsafe extern "C" fn(uintptr_t, *mut c_void) -> Stat, fn_user: *mut c_void) -> Stat;

    fn isl_constraint_list_map(list: uintptr_t, fn_: unsafe extern "C" fn(uintptr_t, *mut c_void) -> uintptr_t, user: *mut c_void) -> uintptr_t;
...
impl ConstraintList {
    ...
    pub fn get_at(&self, index: i32) -> Constraint {
        ...
        let isl_rs_result = Constraint { ptr: isl_rs_result, should_free_on_drop: true };
        isl_rs_result
    }

    /// Wraps `isl_constraint_list_sort`.
    pub fn sort<cmp>(self, mut cmp: cmp, user: *mut c_void) -> ConstraintList
    where cmp: FnMut(&Constraint, &Constraint) -> i32,
    {
        let context_for_error_message = self.get_ctx();
        let list = self;
        let mut list = list;
    ...
    where test: FnMut(&Constraint) -> bool,
    {
        let context_for_error_message = self.get_ctx();
        let list = self;
        let list = list.ptr;
    ...
    where follows: FnMut(&Constraint, &Constraint) -> bool,
          fn_: FnMut(&ConstraintList) -> Stat,
    {
        let context_for_error_message = self.get_ctx();
        let list = self;
        let list = list.ptr;
        ...
        let user: *mut c_void = unsafe { core::mem::transmute(user) };
        let fn_ = fn__wrapper::<fn_>;
        let isl_rs_result = unsafe { isl_constraint_list_foreach_scc(list, follows, follows_user, fn_, fn_user) };
        isl_rs_result
    }

    /// Wraps `isl_constraint_list_map`.
    pub fn map<fn_>(self, mut fn_: fn_, user: *mut c_void) -> ConstraintList
    where fn_: FnMut(&Constraint) -> &Constraint,
    {
        let context_for_error_message = self.get_ctx();
        let list = self;
        let mut list = list;
...
impl Drop for ConstraintList {
    fn drop(&mut self) {
        if self.should_free_on_drop {
            unsafe { isl_constraint_list_free(self.ptr); }
        }
    }
}