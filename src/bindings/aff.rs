...
use libc::uintptr_t;
use crate::bindings::{DimType, LocalSpace, Set, AffList, BasicSet, Val, Space, Id, MultiAff, Context, MultiId, Point};
...
extern "C" {
    ...
    fn isl_aff_set_dim_name(aff: uintptr_t, type_: DimType, pos: u32, s: *const c_char) -> uintptr_t;

    fn isl_aff_lt_basic_set(aff1: uintptr_t, aff2: uintptr_t) -> uintptr_t;

    fn isl_aff_add_coefficient_val(aff: uintptr_t, type_: DimType, pos: i32, v: uintptr_t) -> uintptr_t;
    ...
    fn isl_aff_move_dims(aff: uintptr_t, dst_type: DimType, dst_pos: u32, src_type: DimType, src_pos: u32, n: u32) -> uintptr_t;
    ...
    fn isl_aff_set_coefficient_val(aff: uintptr_t, type_: DimType, pos: i32, v: uintptr_t) -> uintptr_t;
...
impl Aff {
    ...
    pub fn move_dims(self, dst_type: DimType, dst_pos: u32, src_type: DimType, src_pos: u32, n: u32) -> Aff {
        let context_for_error_message = self.get_ctx();
        let aff = self;
        let mut aff = aff;
        aff.do_not_free_on_drop();
        let aff = aff.ptr;
        let isl_rs_result = unsafe { isl_aff_move_dims(aff, dst_type, dst_pos, src_type, src_pos, n) };
        if isl_rs_result == 0 {
            panic!("ISL error: {}", context_for_error_message.last_error_msg());
        }
        let isl_rs_result = Aff { ptr: isl_rs_result, should_free_on_drop: true };
