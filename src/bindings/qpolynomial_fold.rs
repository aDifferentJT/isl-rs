...
use libc::uintptr_t;
use crate::bindings::{Fold, DimType, Stat, Space, Context, Set, Val, Point, QPolynomial};
...
impl QPolynomialFold {
    ...
    pub fn foreach_qpolynomial<fn_>(&self, mut fn_: fn_, user: *mut c_void) -> Stat
...
impl Drop for QPolynomialFold {
    fn drop(&mut self) {
        if self.should_free_on_drop {
            unsafe { isl_qpolynomial_fold_free(self.ptr); }
        }
    }
}