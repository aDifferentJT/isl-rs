...
use libc::uintptr_t;
use crate::bindings::{DimType, Stat, Fold, Space, QPolynomialList, Val, Context, QPolynomialFold, Constraint, Aff, Term, BasicSet, Point, Set};
...
extern "C" {
    ...
    fn isl_qpolynomial_set_dim_name(qp: uintptr_t, type_: DimType, pos: u32, s: *const c_char) -> uintptr_t;
    ...
    fn isl_qpolynomial_foreach_term(qp: uintptr_t, fn_: unsafe extern "C" fn(uintptr_t, *mut c_void) -> Stat, user: *mut c_void) -> Stat;

    fn isl_qpolynomial_as_polynomial_on_domain(qp: uintptr_t, bset: uintptr_t, fn_: unsafe extern "C" fn(uintptr_t, uintptr_t, *mut c_void) -> Stat, user: *mut c_void) -> Stat;
    ...
    fn isl_qpolynomial_move_dims(qp: uintptr_t, dst_type: DimType, dst_pos: u32, src_type: DimType, src_pos: u32, n: u32) -> uintptr_t;
    ...
    fn isl_qpolynomial_fold_foreach_qpolynomial(fold: uintptr_t, fn_: unsafe extern "C" fn(uintptr_t, *mut c_void) -> Stat, user: *mut c_void) -> Stat;
    ...
    fn isl_qpolynomial_fold_move_dims(fold: uintptr_t, dst_type: DimType, dst_pos: u32, src_type: DimType, src_pos: u32, n: u32) -> uintptr_t;
...
impl QPolynomial {
    ...
    pub fn to_list(self) -> QPolynomialList {
        ...
        let isl_rs_result = QPolynomialList { ptr: isl_rs_result, should_free_on_drop: true };
        isl_rs_result
    }
    ...
    pub fn foreach_term<fn_>(&self, mut fn_: fn_, user: *mut c_void) -> Stat
    ...
    pub fn as_polynomial_on_domain<fn_>(&self, bset: &BasicSet, mut fn_: fn_, user: *mut c_void) -> Stat
    ...
    pub fn move_dims(self, dst_type: DimType, dst_pos: u32, src_type: DimType, src_pos: u32, n: u32) -> QPolynomial {
        ...
        qp.do_not_free_on_drop();
        let qp = qp.ptr;
        let isl_rs_result = unsafe { isl_qpolynomial_move_dims(qp, dst_type, dst_pos, src_type, src_pos, n) };
        if isl_rs_result == 0 {
            panic!("ISL error: {}", context_for_error_message.last_error_msg());
        }
        let isl_rs_result = QPolynomial { ptr: isl_rs_result, should_free_on_drop: true };
    ...
    pub fn fold_foreach_qpolynomial<fn_>(fold: &QPolynomialFold, mut fn_: fn_, user: *mut c_void) -> Stat
    ...
    pub fn fold_move_dims(fold: QPolynomialFold, dst_type: DimType, dst_pos: u32, src_type: DimType, src_pos: u32, n: u32) -> QPolynomialFold {
        ...
        fold.do_not_free_on_drop();
        let fold = fold.ptr;
        let isl_rs_result = unsafe { isl_qpolynomial_fold_move_dims(fold, dst_type, dst_pos, src_type, src_pos, n) };
        if isl_rs_result == 0 {
            panic!("ISL error");
        }
        let isl_rs_result = QPolynomialFold { ptr: isl_rs_result, should_free_on_drop: true };
...
impl Drop for QPolynomial {
    fn drop(&mut self) {
        if self.should_free_on_drop {
            unsafe { isl_qpolynomial_free(self.ptr); }
        }
    }
}