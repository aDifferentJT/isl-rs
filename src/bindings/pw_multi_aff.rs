...
use crate::bindings::{DimType, Stat, Space, MultiAff, UnionPwMultiAff, MultiVal, Set, Context, Id, Map, PwAff, MultiId, MultiPwAff, Val, PwMultiAffList};
...
extern "C" {
    ...
    fn isl_pw_multi_aff_find_dim_by_name(pma: uintptr_t, type_: DimType, name: *const c_char) -> i32;

    fn isl_pw_multi_aff_union_lexmin(pma1: uintptr_t, pma2: uintptr_t) -> uintptr_t;

    fn isl_pw_multi_aff_every_piece(pma: uintptr_t, test: unsafe extern "C" fn(uintptr_t, uintptr_t, *mut c_void) -> i32, user: *mut c_void) -> i32;
    ...
    fn isl_pw_multi_aff_set_dim_id(pma: uintptr_t, type_: DimType, pos: u32, id: uintptr_t) -> uintptr_t;
    ...
    fn isl_pw_multi_aff_project_out_map(space: uintptr_t, type_: DimType, first: u32, n: u32) -> uintptr_t;
    ...
    fn isl_pw_multi_aff_foreach_piece(pma: uintptr_t, fn_: unsafe extern "C" fn(uintptr_t, uintptr_t, *mut c_void) -> Stat, user: *mut c_void) -> Stat;
    ...
    fn isl_pw_multi_aff_preimage_domain_wrapped_domain_pw_multi_aff(pma1: uintptr_t, pma2: uintptr_t) -> uintptr_t;
    ...
    fn isl_pw_multi_aff_intersect_domain_wrapped_domain(pma: uintptr_t, set: uintptr_t) -> uintptr_t;

    fn isl_pw_multi_aff_intersect_domain_wrapped_range(pma: uintptr_t, set: uintptr_t) -> uintptr_t;
...
impl PwMultiAff {
    ...
    where test: FnMut(&Set, &MultiAff) -> bool,
    {
        let context_for_error_message = self.get_ctx();
        let pma = self;
        let pma = pma.ptr;
    ...
    where fn_: FnMut(&Set, &MultiAff) -> Stat,
    {
        let context_for_error_message = self.get_ctx();
        let pma = self;
        let pma = pma.ptr;
    ...
    pub fn preimage_domain_wrapped_domain_pw_multi_aff(self, pma2: PwMultiAff) -> PwMultiAff {
        ...
        pma2.do_not_free_on_drop();
        let pma2 = pma2.ptr;
        let isl_rs_result = unsafe { isl_pw_multi_aff_preimage_domain_wrapped_domain_pw_multi_aff(pma1, pma2) };
        if isl_rs_result == 0 {
            panic!("ISL error: {}", context_for_error_message.last_error_msg());
        }
        let isl_rs_result = PwMultiAff { ptr: isl_rs_result, should_free_on_drop: true };
...
impl Drop for PwMultiAff {
    fn drop(&mut self) {
        if self.should_free_on_drop {
            unsafe { isl_pw_multi_aff_free(self.ptr); }
        }
    }
}